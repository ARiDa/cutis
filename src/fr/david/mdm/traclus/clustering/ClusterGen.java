...
import fr.david.mdm.traclus.clustering.TraClusterDoc.Parameter;
...
public class ClusterGen {
	...
	public ArrayList<Integer> m_componentIdArray = new ArrayList<Integer>();
	// the list of line segment clusters
	public LineSegmentCluster[] m_lineSegmentClusters;
	...
	public ArrayList<CMDLineSegment> m_lineSegmentPointArray = new ArrayList<CMDLineSegment>();
	...
	private static final double  MDL_COST_ADWANTAGE=0;
	...
	public <T> ClusterGen(TraClusterDoc document, DirectionSpaceTimeDistanceTraclus dm) {
	...
	private boolean storeClusterComponentIntoIndex() {
		...
		for(int i=0; i<m_document.m_trajectoryList.size(); i++) {
			Trajectory pTrajectory = (Trajectory) m_document.m_trajectoryList.get(i);
			...
			for(int j =0; j<pTrajectory.getM_nPartitionPoints()-1; j++) {
				...
				CMDLineSegment lineSegmentPoint = new CMDLineSegment(nDimensions);
				lineSegmentPoint.setM_CMDPoint(0, startPoint);
				lineSegmentPoint.setM_CMDPoint(1, endPoint);
	...
	private void findOptimalPartition(Trajectory pTrajectory) {
		...
		double fullPartitionMDLCost, partialPartitionMDLCost;
		...
		for(;;) {
			...
			for(length = 1; startIndex+length < nPoints; length++) {
				...
				fullPartitionMDLCost = computeModelCost(pTrajectory, startIndex+length-1, startIndex+length); //costnopar //preserving the original trajectory 
				
				//compute the sum of (1) the length of a cluster component and 
				//					 (2) the perpendicular and angle distances
				partialPartitionMDLCost = computeModelCost(pTrajectory, startIndex+length-1, startIndex+length) +
						computeEncodingCost(pTrajectory, startIndex+length-1, startIndex + length); //costpar
				if(fullPartitionMDLCost + MDL_COST_ADWANTAGE < partialPartitionMDLCost) { //this constant is to make trajectory partitions longer
										
					if(startIndex==startIndex + length - 1){
						//add the end point of a trajectory
						//System.out.println(pTrajectory.getM_trajectoryId());
	...
	//compute L(H)
	private double computeModelCost(Trajectory pTrajectory, int startPIndex, int endPIndex) {
		...
		//double distance = measureDistanceFromPointToPoint(lineSegmentStart, 
			//	lineSegmentEnd); //euclidian distance
		
		//TODO by me
		
		double distance = dm.length(lineSegmentStart,lineSegmentEnd);
		//return distance;
	...
	private int computeEncodingCost(Trajectory pTrajectory, int startPIndex,
		...
		for(int i = startPIndex; i< endPIndex; i++) {
			...
			//TODO by me
			perpendicularDistance = dm.distanceSpatialTime(clusterComponentStart,
					clusterComponentEnd, lineSegmentStart, lineSegmentEnd);
			
			angleDistance = dm.distanceAngular(clusterComponentStart,
					clusterComponentEnd, lineSegmentStart, lineSegmentEnd);
			...
			//encodingCost += perpendicularDistance+angleDistance;
...
//		m_projectionPoint = new CMDPoint();
		...
		// a coefficient (0 <= b <= 1)
		m_coefficient = computeInnerProduct(m_vector1, m_vector2) / computeInnerProduct(m_vector2, m_vector2); //compute u1
		...
		for (int i = 0; i < nDimensions; i++)
			m_projectionPoint.setM_coordinate(i, s.getM_coordinate(i) + m_coefficient * m_vector2.getM_coordinate(i)); //compute ps
		...
		for (int i = 0; i < m_nTotalLineSegments; i++)
		{
			...
			if (componentId >= 0) {
				for (int j = 0; j < nDimensions; j++) {
					...
					double difference = m_lineSegmentPointArray.get(i).getM_CMDPoint(1).getM_coordinate(j)
						- m_lineSegmentPointArray.get(i).getM_CMDPoint(0).getM_coordinate(j);
					double currSum = m_lineSegmentClusters[componentId].avgDirectionVector.getM_coordinate(j)
						+ difference;
					m_lineSegmentClusters[componentId].avgDirectionVector.setM_coordinate(j, currSum);
				}
	...
	private void computeRepresentativeLines(LineSegmentCluster clusterEntry) {
		...
		while(iter != (clusterEntry.candidatePointList.size()-1) && clusterEntry.candidatePointList.size()>0)	{
			...
			{
				if (Math.abs(candidatePoint.orderingValue - prevOrderingValue) > 0)//((double)MIN_LINESEGMENT_LENGTH / 1.414))
				{
	...
	private void getSweepPointOfLineSegment(LineSegmentCluster clusterEntry,
		...
		CMDLineSegment lineSegmentPoint = m_lineSegmentPointArray.get(lineSegmentId);		// 2n-dimensional point
		double coefficient;
		
		// NOTE: this program code works only for the 2-dimensional data
		double newStartX, newEndX, newStartY, newEndY;
		newStartX = GET_X_ROTATION(lineSegmentPoint.getM_CMDPoint(0).getM_coordinate(0), lineSegmentPoint.getM_CMDPoint(0).getM_coordinate(1), clusterEntry.cosTheta, clusterEntry.sinTheta);
		newEndX   = GET_X_ROTATION(lineSegmentPoint.getM_CMDPoint(1).getM_coordinate(0), lineSegmentPoint.getM_CMDPoint(1).getM_coordinate(1), clusterEntry.cosTheta, clusterEntry.sinTheta);
		newStartY = GET_Y_ROTATION(lineSegmentPoint.getM_CMDPoint(0).getM_coordinate(0), lineSegmentPoint.getM_CMDPoint(0).getM_coordinate(1), clusterEntry.cosTheta, clusterEntry.sinTheta);
		newEndY   = GET_Y_ROTATION(lineSegmentPoint.getM_CMDPoint(1).getM_coordinate(0), lineSegmentPoint.getM_CMDPoint(1).getM_coordinate(1), clusterEntry.cosTheta, clusterEntry.sinTheta);
	...
	private void RegisterAndUpdateLineSegmentCluster(int componentId, int lineSegmentId) {
		...
		CMDLineSegment aLineSegment = m_lineSegmentPointArray.get(lineSegmentId);
		double orderingValue1 = GET_X_ROTATION(aLineSegment.getM_CMDPoint(0).getM_coordinate(0),
				aLineSegment.getM_CMDPoint(0).getM_coordinate(1), clusterEntry.cosTheta, clusterEntry.sinTheta); 
		double orderingValue2 = GET_X_ROTATION(aLineSegment.getM_CMDPoint(1).getM_coordinate(0), 
				aLineSegment.getM_CMDPoint(1).getM_coordinate(1), clusterEntry.cosTheta, clusterEntry.sinTheta); 
	...
	private void computeEPSNeighborhood(CMDPoint startPoint,
		...
		for(int j=0; j<m_nTotalLineSegments; j++) {
			...
			//TODO by me
			double distance = dm.distance(startPoint,endPoint,m_startPoint2,m_endPoint2);
	...
	private void extractStartAndEndPoints(int index, CMDPoint startPoint,
		...
		startPoint.setTime(m_lineSegmentPointArray.get(index).getM_CMDPoint(0).m_time);
		endPoint.setTime(m_lineSegmentPointArray.get(index).getM_CMDPoint(1).m_time);
		for(int i=0; i< m_document.m_nDimensions;i++) {
			//double a = m_lineSegmentPointArray.get(index).getM_coordinate(i);
			startPoint.setM_coordinate(i, m_lineSegmentPointArray.get(index).getM_CMDPoint(0).getM_coordinate(i));
			endPoint.setM_coordinate(i, m_lineSegmentPointArray.get(index).getM_CMDPoint(1).
					getM_coordinate(i));
		}
